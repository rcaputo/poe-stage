...
and structure.
	...
	<main>
		<object role="application">
			<object role="http client">
				<object role="dns client">
					<object role="worker"/>
					<object role="worker"/>
					<object role="worker"/>
				</object>
				<object role="keepalive connector">
					<object role="socket"/>
					<object role="socket"/>
				</object>
				<object role="request"/>
...
regardless of path.
	...
	my @workers = Object->find('//object[@role="worker"]');
	my @http_requests = Object->find(
		'//object[@role="http_client"]/object[@role="request"]'
	);
	my @http_clients = Object->find(
		'/main/object[@role="application"]/object[@role="http client"]'
	);
...
We can now use shorter notation.
...
While XPath may not be the right choice, it at least comes close to
eliminating POE::Kernel's aliases' restrictions.

Storing the program structure in a DOM also allows it to be serialized
and shared trivially.  It could be

=head2 Connections as Helpers

Helper objects may be a way to represent connections between objects.
These helpers would be proxy interfaces for the hidden systems that
connect objects between processes.
...
=head3 XXX AM HERE XXX TRANSCRIBE EXAMPLES

I<Todo:> Transcribe examples from paper notes.
