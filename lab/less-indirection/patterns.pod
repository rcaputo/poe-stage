=head1 Object Patterns and Framework

This document intends to be two things.  First, it presents an
exhaustive catalog of runtime object use and interaction patterns.
Second, it builds upon them an abstract set of specifications for an
object framework, focusing mainly on runtime usage and effects.

The framework is not named, in order to keep discussion abstract.
...
=head2 Design Roadmap

This document will be developed in alternating phases of expansionary
braindumps and consolidating edits.  Braindumps will expand the
document's breadth at the expense of overall depth.  Edits will deepen
the good concepts while explaining away the bad, making the document
...
=item * Restructure into descriptive and prescriptive sections.

=back

=head2 Contributing

You can help by adding your braindumps and discussing features in more
depth.  The author is not omniscient.  Useless features will be
documented and (hopefully) discarded.  Important considerations will
be omitted.  Everyone's priorities are different, so what matters to
one reader may not be important to the author.  The only way to be
sure is to speak up.

=head1 Object Categories

=head2 Helper Objects
...
An object may be transferred by calling a method from the new owner.
The object's new parent would call the method on the object being
claimed.  This take_object() method might be implemented on the
parent.
...
Implicit management provides an API where one object declares
ownership of another.  This may be used to transfer ownership as well
as initially set it.  The proposed detach() method is poorly named for
this purpose.
...
described as services earlier in this document.  POE::Session is a
kind of actor, as are POE::Component objects.  POE::Wheel objects
aren't really actors because they colonize the sessions that create
them.
...
=head3 JavaScript use of DOM

Torsten Raudssus commented that Javascript uses the DOM for similar
purposes.  Code may use XPath to manipulate page elements as groups.
It can be very convenient.

However this case seems to be a subset of Robert Grimes' earlier
point.  Javascript is manipulating sibling objects within the same
container---the page containing the code.

In a larger application, it seems safer to limit each object's reach.
The next question in this line of reasoning is: What sorts of
...
=head2 Reflexive Data Members

POE's early object environment experiments were very MUD-like.  Nearly
every object required a container---another object within which it
existed.  Changing an object's place required updates in its old and
new containers.

This was such a fundamental aspect of the system that it defined a
reflexive attribute trait.  For example, one class's "place" member
could be flagged as the reflexive data for another's "contents".
Storing an object ID in $place would do a few things:

=over 2

=item * Ask the object whether changing $place was allowed.

=item * Ask the old $place whether the object could be removed.

=item * Ask the new $place whether the object could be added.

=item * Remove the object from the old $place.

=item * Add the object to the new $place.

=item * Finally, change the object's $place.

=back

This was slow, but it guaranteed fundamental referential integrity in
the system.

The new framework can do similar, although encapsulated and hidden in
the implicit object management API.
...
Chris Williams suggested Variable::Magic for watching POE HEAP or
object data members.
